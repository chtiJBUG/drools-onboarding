 ...
 */
public class ChtiJbugClassPathKieProject extends AbstractKieProject {

    private static final Logger log = LoggerFactory.getLogger(org.drools.compiler.kie.builder.impl.ClasspathKieProject.class);
    ...
    ChtiJbugClassPathKieProject(ClassLoader parentCL, WeakReference<KieServicesEventListerner> listener) {
    ...
    private static String getPomPropertiesFromFileSystem(String rootPath) {
        Reader reader = null;
        try {
            File file = ChtiJbugKieBuilderImpl.findPomProperties(new File(rootPath));
            if (file == null) {
                log.warn("Unable to find pom.properties in " + rootPath);
                return null;
            }
