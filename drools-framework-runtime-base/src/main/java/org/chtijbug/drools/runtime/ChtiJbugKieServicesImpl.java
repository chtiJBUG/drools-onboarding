...
public class ChtiJbugKieServicesImpl implements InternalKieServices {
    ...
    public KieContainer newKieClasspathContainer(String containerId, ClassLoader classLoader) {
        if (containerId == null) {
            KieContainerImpl newContainer = new KieContainerImpl(UUID.randomUUID().toString(), new ChtiJbugClassPathKieProject(classLoader, listener), null);
            return newContainer;
        }
        if (kContainers.get(containerId) == null) {
            KieContainerImpl newContainer = new KieContainerImpl(containerId,  new ChtiJbugClassPathKieProject(classLoader, listener), null);
    ...
    public KieBuilder newKieBuilder(File file) {
        return file.isDirectory() ? new ChtiJbugKieBuilderImpl(file) : newKieBuilder(new KieFileSystemImpl(readFromJar(file)));
    }

    public KieBuilder newKieBuilder(KieFileSystem kieFileSystem) {
        return new ChtiJbugKieBuilderImpl(kieFileSystem);
    }

    public KieBuilder newKieBuilder(KieFileSystem kieFileSystem, ClassLoader classLoader) {
        return new ChtiJbugKieBuilderImpl(kieFileSystem, classLoader);
    }
