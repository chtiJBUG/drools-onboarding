...
public class ChtiJbugKieBuilderImpl
        implements
        InternalKieBuilder {
    ...
    private ChtijbugKieBuilderSetImpl kieBuilderSet;

    private ClassLoader classLoader;

    private PomModel pomModel;

    public ChtiJbugKieBuilderImpl(File file) {
        this.srcMfs = new DiskResourceReader(file);
    }

    public ChtiJbugKieBuilderImpl(KieFileSystem kieFileSystem) {
    ...
    public KieBuilder buildAll() {
        ...
        if (!isBuilt() && kModuleModel != null) {
            ...
            for (ReleaseId unresolvedDep : kModule.getUnresolvedDependencies()) {
                results.addMessage(Message.Level.ERROR, "pom.xml", "Unresolved dependency " + unresolvedDep);
            }
    ...
    private KieModule getKieModule(boolean ignoreErrors) {
        ...
        if (!ignoreErrors && (getResults().hasMessages(Message.Level.ERROR) || kModule == null)) {
    ...
    private void buildKieModuleModel() {
        if (srcMfs.isAvailable(KieModuleModelImpl.KMODULE_SRC_PATH)) {
            ...
            try {
                ...
                results.addMessage(Message.Level.ERROR,
                        "kmodule.xml",
        ...
        for (String fileName : srcMfs.getFileNames()) {
            if (fileName.endsWith(".java") && !classFiles.contains(fileName.substring(0,
                ...
                if (!fileName.startsWith(JAVA_ROOT) && !fileName.startsWith(JAVA_TEST_ROOT)) {
                    results.addMessage(Message.Level.WARNING, fileName, "Found Java file out of the Java source folder: \"" + fileName + "\"");
                } else if (fileName.substring(JAVA_ROOT.length()).indexOf('/') < 0) {
                    results.addMessage(Message.Level.ERROR, fileName, "A Java class must have a package: " + fileName.substring(JAVA_ROOT.length()) + " is not allowed");
                } else {
    ...
    public KieBuilderSet createFileSet(String... files) {
        if (kieBuilderSet == null) {
            kieBuilderSet = new ChtijbugKieBuilderSetImpl(this);
        }
        return kieBuilderSet.setFiles(files);
    }

    public IncrementalResults incrementalBuild() {
        return new ChtijbugKieBuilderSetImpl(this).build();
    }
}
